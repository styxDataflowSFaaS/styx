...
SNAPSHOT_TYPE = "Aligned"  # "Unaligned" unaligned should be with a write ahead log and sequence t_ids
...
class UnsafeProtocol(BaseTransactionalProtocol):
    ...
    async def wait_align_start_sync(self):
        await self.send_sync_to_peers(100)  # ALIGN_START
    ...
    async def wait_align_end_sync(self):
        await self.send_sync_to_peers(101)  # ALIGN_END
        wait_remote_fallback = [event.wait()
